 ...
 *
 */
function ie_shared_counts_header( $output, $location ) {
	if( 'after_content' === $location )
		$output = '<h3>Share this Article</h3>' . $output;
	return $output;
}
add_filter( 'shared_counts_display', 'ie_shared_counts_header', 10, 2 );

/**
 * Simple email button
 * Does not require loading JS
 */
function ie_shared_counts_email_link( $link, $id ) {
	if( 'email' !== $link['type'] )
		return $link;
...
add_filter( 'shared_counts_link', 'ie_shared_counts_email_link', 10, 2 );

/**
 * Shared Counts Services
 *
 */
function ie_shared_counts_services( $services, $location ) {
	if( 'after_content' !== $location )
		return $services;
...
add_filter( 'shared_counts_display_services', 'ie_shared_counts_services', 10, 2 );

/**
 * Shared Counts Locations
 *
 */
function ie_shared_counts_locations( $locations ) {
	$locations['before']['hook'] = 'ie_entry_header_share';
	$locations['after']['hook'] = 'ie_entry_footer_share';
	$locations['after']['style'] = 'button';
	return $locations;
}
add_filter( 'shared_counts_theme_locations', 'ie_shared_counts_locations' );
 ...
 *
 */
function ie_production_url( $url = false ) {
 ...
 * @return array
 */
function ie_production_url_share_count_api( $params ) {
	$params['url'] = ie_production_url( $params['url'] );
	return $params;
}
add_filter( 'shared_counts_api_params', 'ie_production_url_share_count_api' );
 ...
 * @return array
 */
function ie_production_url_share_count_link( $link ) {
	$exclude = array( 'print', 'email' );
	if( ! in_array( $link['type'], $exclude ) )
		$link['link'] = ie_production_url( $link['link'] );
	return $link;
}
add_filter( 'shared_counts_link', 'ie_production_url_share_count_link' );
