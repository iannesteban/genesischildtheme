 ...
 */
function ie_amp_class( $default, $active, $state ) {
	$output = '';
	if( ie_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function ie_amp_toggle( $state = '', $disable = array() ) {
	if( ! ie_is_amp() )
		return;
 ...
 */
function ie_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return ie_amp_toggle( $key ) . ie_amp_class( 'submenu-expand', 'expanded', $key );
}
