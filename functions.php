...
function ie_global_enqueues() {
	...
	wp_enqueue_style( 'ea-fonts', ie_theme_fonts_url() );
...
add_action( 'wp_enqueue_scripts', 'ie_global_enqueues' );

/**
 * Gutenberg scripts and styles
 *
 */
function ie_gutenberg_scripts() {
	wp_enqueue_style( 'ea-fonts', ie_theme_fonts_url() );
...
add_action( 'enqueue_block_editor_assets', 'ie_gutenberg_scripts' );

/**
 * Theme Fonts URL
 *
 */
function ie_theme_fonts_url() {
	return false;
}
...
function ie_child_theme_setup() {
	...
	// Image Sizes
	// add_image_size( 'ie_featured', 400, 100, true );
	...
	add_theme_support( 'editor-font-sizes', array(
		array(
			'name'      => __( 'Small', 'ie_genesis_child' ),
			'shortName' => __( 'S', 'ie_genesis_child' ),
			'size'      => 14,
			'slug'      => 'small'
		),
		array(
			'name'      => __( 'Normal', 'ie_genesis_child' ),
			'shortName' => __( 'M', 'ie_genesis_child' ),
			'size'      => 20,
			'slug'      => 'normal'
		),
		array(
			'name'      => __( 'Large', 'ie_genesis_child' ),
			'shortName' => __( 'L', 'ie_genesis_child' ),
			'size'      => 24,
			'slug'      => 'large'
		),
	) );
	...
	add_theme_support( 'editor-color-palette', array(
		array(
			'name'  => __( 'Blue', 'ie_genesis_child' ),
			'slug'  => 'blue',
			'color'	=> '#05306F',
		),
		array(
			'name'  => __( 'Grey', 'ie_genesis_child' ),
			'slug'  => 'grey',
			'color' => '#FAFAFA',
		),
	) );
...
add_action( 'genesis_setup', 'ie_child_theme_setup', 15 );
 ...
 * @return array
 */
function ie_comment_text( $args ) {
	$args['title_reply']          = __( 'Leave A Reply', 'ie_genesis_child' );
	$args['label_submit']         = __( 'Post Comment',  'ie_genesis_child' );
	$args['comment_notes_before'] = '';
	$args['comment_notes_after']  = '';
	return $args;
}
add_filter( 'comment_form_defaults', 'ie_comment_text' );
 ...
 *
 */
function ie_template_hierarchy( $template ) {
	if( is_home() )
		$template = get_query_template( 'archive' );
	return $template;
}
add_filter( 'template_include', 'ie_template_hierarchy' );
